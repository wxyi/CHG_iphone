...
- (id)SKObjectForKey:(id)aKey
{
//    SK_TRY_BODY([self SKObjectForKey:aKey];)
//    DLog(@"[self SKObjectForKey:aKey] = %@",[self SKObjectForKey:aKey]);
//    id obj = [self SKObjectForKey:aKey];
//    if ([obj isKindOfClass:[NSNull class]]) {
//        return @"";
//    }
    SK_TRY_BODY(return [self SKObjectForKey:aKey];)
    return @"";
}
+ (void) load{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [self swizzleMethod:@selector(SKObjectForKey:) tarClass:@"__NSDictionaryM" tarSel:@selector(objectForKey:)];
    });
}
@end
